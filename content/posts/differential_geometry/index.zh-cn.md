...
d. 在第四章中，我们引入两种导数，即协变导数和李导数。协变导数描述了流形上的联络，并可以用来定义平移、测地线和曲率。基于这些概念，我们就可以简要介绍广义相对论；
...
>
> 我们说线性变换是一个 $(1,1)$ 型张量，这实际上是在说 $\mathrm{End}(V)$ 和 $\mathcal{T}^1_1(V)$ 之间有一个自然同构。这个自然同构定义如下：
>
> $$ 
> \begin{aligned}
> f: \mathcal{T}^1_1(V) &\rightarrow \mathrm{End}(V) \\\\
> T &\mapsto A \\\\
> T(\omega, X)&=\omega(AX)
> \end{aligned}
> $$
> 
> 其中 $A\in \mathrm{End}(V)$ 是一个 $V$ 上的线性变换，$T$ 是一个 $(1,1)$ 型张量，$X\in V$，$\omega \in V^*$。
>
