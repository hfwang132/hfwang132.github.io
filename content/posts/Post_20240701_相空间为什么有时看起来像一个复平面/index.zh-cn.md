...
aliases:
  - "/phase_space_complex/"
  - "/Post_20240727_相空间为什么有时看起来像一个复平面/"
draft: false
tags: ["量子光学"]
categories: ["量子信息"]
math: true
...
这些种种都在暗示我们：难道相空间可以是个复平面？经过与数学系同学的讨论，的确如此，但**前提是该相空间自身是一个向量空间**。论证如下：
...
**（4）**一个带内积 \(g\) 的辛向量空间 \((V,\omega)\) 自带一个典范的复结构 \(J\) [[1]](#ref\_1)。
...
我们可以借助 \(g(u,v)=\omega(u,Jv)\) 定义复结构 \(J\in \text{End}[T^*(\mathbb{R})]\) 。由于其矩阵形式满足 \(M(g)=M(\omega)M(J)\) ，所以 \(M(J)=\begin{pmatrix} 0 & -1\\ 1 & 0 \end{pmatrix}\) 。这个矩阵是不是有点眼熟？没错，它就是虚数单位 \(\mathrm{i}\) 的矩阵表示。
