...
b. 在第二章中，我们定义流形上的切矢量场、余切矢量场和张量场。我们还定义内积和度规张量，它们是流形上的额外结构。  
  
c. 在第三章中，我们引入外积、外代数等概念，并在流形上研究外微分。最后，我们介绍 Hodge 对偶。  
  
d. 在第四章中，我们引入两种导数，即协变导数和李导数。

协变导数描述了流形上的联络，并可以用来定义平移、测地线和曲率。基于这些概念，我们就可以简要介绍广义相对论；

李导数描述了流形上向量场的“流动”，它非常有用，可以用来简洁地定义散度、挠率、曲率等等概念。它还能引出李代数和李群，不过那就是另外一个话题了，本文暂不介绍。  
...
本文主要参考 Differential Geometry and Lie Groups for Physicists, by Marián Fecko

b. **目标读者**

本文需要的前置知识有：初等微积分、初等线性代数。

如果读者是**物理系**的，那么效果最佳。本文**适合物理系大二及以上的学生**阅读。

如果读者是数学系的，也完全欢迎阅读、提出意见、指出错误。

严谨性与易懂性永远是对立互补的。本文为了易懂，难免在语言上牺牲一些严谨性。因此请数学系读者包容。

c. **阅读建议**

本文旨在**梳理概念**，不能作为教程来阅读。当然，如果你只是感兴趣，那么阅读本文绝对是没有坏处的。
...
>   
> **例 1.2**  
> 欧几里得空间 $\mathbb{R}^n$ 中的所有开集构成它的一个拓扑。
...
> **评论**  
>   
> 在一般的微积分课程中，我们先是在 $\mathbb{R}^n$ 上定义了一个度量，使其成为欧几里得空间，再通过度量来定义开集的概念。  
>   
> 拓扑的概念使得我们不需要依赖度量也可以定义开集。在拓扑学中，我们将分析学里的开集的“有限交”和“无限并”的性质抽象出来，用来定义开集本身。  
>   
> 如果上下文允许，我们可以直接将 $X$ 叫做拓扑空间，而不用将 $(X,\mathcal{F})$ 写出来。  
>   
> **选读 1.2**  
>   
> 你可能想问，为什么我们不用“可数交”和“有限并”来定义闭集，然后以此来定义拓扑呢？为什么选择开集，而不是闭集？这可能涉及各种历史因素，详见这个[问题](https://www.zhihu.com/question/396132316)。

### 1.3 连续（Continuity）  

设 $(X,\mathcal{F})$ 和 $(Y,\mathcal{S})$ 是拓扑空间， $f:X\rightarrow Y$ 是一个映射。如果 $\forall O \in \mathcal{S}$ 都有 $f^{-1}[O] \in \mathcal{F}$ ，则称 $f$ 是连续的。其中 $f^{-1}[O]$ 代表 $O$ 在 $f$ 下的原像（Inverse Image）。
...
若 $f:X\rightarrow Y$ 是**双射**，且 $f$ 及其反函数都是连续的，则称 $f$ 是从 $X$ 到 $Y$ 的一个**同胚**。

如果存在从 $X$ 到 $Y$ 的同胚，那么我们也可以说， $X$ 与 $Y$ 同胚。

> **评论**   
>   
> 当我们说一个（带把的）咖啡杯与一个甜甜圈同胚的时候，我们实际在说二者的点可以建立一一对应关系，也就是双射，并且这个双射是双连续的。  
>   
...
> **评论**  
>   
> 数学家会搞出一些奇奇怪怪的非豪斯多夫空间，但对于我们物理人来说，豪斯多夫空间是天经地义的。我们后面介绍的流形，在定义上都属于豪斯多夫空间。

### 1.7 拓扑流形（Topological Manifold）  

若豪斯多夫拓扑空间 $M$ 有可数开覆盖 $M=\bigcup_\alpha O_\alpha$（ $O_\alpha$ 为开集），且每一个 $O_\alpha$ 都同胚于 $\mathbb{R}^n$ 的一个开子集（这个 $\mathbb{R}^n$ 的拓扑是我们在初等微积分中学过的欧几里得拓扑），则称 $M$ 为一个 $n$ 维的拓扑流形。

> **评论**  
>   
> 简单来说，如果 $M$ 在每一个**局部**都同胚于（欧几里得意义上的） $\mathbb{R}^n$ 的一个开子集，则将 $M$ 称为拓扑流形。  
>   
> 这里所说的“欧几里得意义上的拓扑”指的是我们在初等微积分课程中都学过的那种用欧式度量所定义的点集拓扑。由于本文假定读者学过初等微积分，因此不再赘言。  
>   
...
> **评论**  
>   
> 在很多书籍中， $\varphi_\alpha$ 的方向是从 $\mathbb{R}^n$ 的开子集到流形 $M$ 的，即 $\varphi_\alpha:V_\alpha\subseteq \mathbb{R}^n\rightarrow M$ ，与我们上文定义的方向相反。这样一来，坐标变换就会写成 $\varphi_\beta^{-1}\circ\varphi_\alpha$ ，而不是 $\varphi_\beta\circ\varphi_\alpha^{-1}$ 。不过，由于 $\varphi$ 是一个双射，所以它的方向并不重要。读者从上下文中自行判断方向即可。
...
>   
> 这就是我们通常所说的“微分结构”，即坐标变换不改变函数的可微性。

### 小结  

在本章中，我们定义了微分流形以及流形上的标量场。步骤如下：

首先，我们定义一个集合的**拓扑**是它的所有开子集构成的**集合**。定义了一个**拓扑**之后，一个集合就不再是一个简单的集合，而是一个拓扑空间。
...
> 可以证明，这与我们之前的定义是等价的。  
>   
> 这两种定义方式都使得切空间不需要镶嵌在另一个更大的空间中，而是可以视作微分流形的内蕴性质。我们只需要审视流形自身，就可以意识到切空间的存在。换句话说，虽然将切空间看做切平面非常形象，但在数学上，我们完全可以抛弃这个图像。  
>   
...
（光滑）向量场定义为从 $\mathcal{F}(M)$ 到 $\mathcal{F}(M)$ 的映射，它把一个（光滑）标量场映射成另外一个（光滑）标量场。

换句话说，它在流形上的每一点都对（光滑）标量场都求了一个方向导数，然后把这些方向导数的值作为一个新的标量场。如果新的标量场也是光滑的，则称该向量场是光滑的。

我们可以把 $M$ 上的（光滑）向量场构成的集合记为 $\mathcal{T}_0^1(M)$ 。这个符号的上标和下标的意思，读者以后会明白。

向量场还有一种等价的定义方式，它借助切丛的概念来完成。
...
流形 $M$ 上的（切）向量场定义为其切丛 $TM$ 的一个截影。如果这个截影是光滑的，那么我们则称向量场是光滑的。

切丛上所有截影的集合可以记为 $ \Gamma(TM)$ 。

> **评论**  
>   
> 你可能想问截影的光滑性，也就是映射 $M\rightarrow TM$ 的光滑性是如何定义的。实际上，如果 $M$ 是一个 $n$ 维流形，则 $TM$ 是一个 $2n$ 维流形，而从流形到流形的映射的光滑性，我们在**第 1.8 节**已经定义过了。  
>   
> 至于为什么 $TM$ 是一个 $2n$ 维流形，涉及到纤维丛的知识，此处暂不介绍。  
>   
> 在数学书中，切丛的截影的定义还要更严谨一些，要先做一个投影 $\pi :(p,v)\mapsto p$ ，再让截影 $\sigma$ 与投影 $\pi$ 的复合成为 $M$ 上的恒等映射 $\sigma \circ \pi = \text{id}$ 。不过，这些技术细节对于我们理解物理图像并没有什么特别的帮助，因此就不细说了。  
>   
> 我们物理人只要把向量场想象成 “为流形上的每一点指派一个该点处的向量” 即可。  
>   
> **评论**  
>   
> 实际上， $TM$ 可以作为拉格朗日力学的舞台。回想一下，拉格朗日力学的变量是广义坐标和广义速度 $x^1,\cdots,x^n,v^1,\cdots,v^n$ 。广义速度其实就是切空间中的元素，于是有 $(x,v)\in TM$ 。  
>   
> **评论**  
>   
...
更具体地说，线性泛函是一个映射 $\omega:V\rightarrow \mathbb{R}$ ，且满足线性性： $\omega(\lambda a + \mu b) = \lambda \omega(a) + \mu \omega(b)$。它将向量空间 $V$ 中的元素映射成一个实数。
...
> **评论**    
>   
> 我们在线性代数中学过，对于任何两个有限维的向量空间，只要它们维数相等，那么它们就是同构的。这是因为只要将在两个向量空间中分别选取一组基，并将这些基一一对应起来，就能得到一个同构。但是，这样的同构一般有无穷多个，因为它们依赖于基的选取，没有哪一个同构是特别的。  
>   
> 不过，我们上面定义的自然同构是唯一且“自然”的：它不依赖于基的选取。  
>   
> 当两个向量空间自然同构时，我们可以把这两个空间看作是同一个空间。换句话说，一个有限维向量空间的对偶空间的对偶空间就是该向量空间自身。  
>   
> 当然，以上只考虑了有限维的线性空间。对于无限维的线性空间来说， $V^*$ 和 $V^{**}$ 比 $V$ 大。不过现在我们暂时不关心这件事。  
>   
> 另外，自然同构也可以称作典范同构（Canonical Isomorphism）。

### 2.4 余切空间、余切丛（Cotangent Space, Cotangent Bundle）  

余切空间定义为切空间的对偶空间，记作 $T_p^*M$ 。余切空间中的元素可以叫做余切向量。

与**第 2.2 节**同理，我们可以把余切向量场定义为：从向量场 $\mathcal{T}^1_0(M)$ 到标量场 $\mathcal{F}(M)$ 的映射。并且将其记作 $\mathcal{T}^0_1(M)$ 。

余切空间仅仅是相对于流形上的某一点来说的。为了研究流形上每一点的余切空间，我们可以参照**第 2.2 节**中定义切丛的方式，定义一个余切丛： $T^*M:=\{(p,\omega)\mid\,p\in M,\ \omega \in T^*_pM\}$，它是流形上所有点与该点处的余切向量的有序对的集合。

余切丛的截影定义为这样一个映射： $M\rightarrow T^*M$ ，它将点 $p$ 映射为 $(p,\omega)$ ，其中 $\omega\in T_p^*M$ 。于是流形 $M$ 上（光滑的）余切向量场也可以定义为余切丛 $T^*M$ 的（光滑）截影。

我们把余切丛上所有（光滑）截影的集合记作 $\Gamma(T^*M)$ 。
...
点 $p$ 处的微分所在的余切空间有什么样的基呢？以 3 维流形上的一点 $p$ 为例，该点的切空间有三个基 $\{\frac{\partial}{\partial x^1},\frac{\partial}{\partial x^2},\frac{\partial}{\partial x^3}\}$ ，可以简记为 $\{\partial_1,\partial_2,\partial_3\}$ 。根据**定理 2.1**，我们可以找到这三个基在余切空间中的对偶基 $\{\mathrm{d}x^1,\mathrm{d}x^2,\mathrm{d}x^3\}$ ，使得 $\mathrm{d}x^j\left(\partial_i\right)=\delta_i^j$ 。某点处的微分可以表示成这些基的线性组合： $\mathrm{d}f=a_i\mathrm{d}x^i$ 。
...
>   
>  可以发现，雅可比矩阵给出了余切空间的基的坐标变换公式。请记住这一点，在后面的**第 2.8-9 节**中会用到。
...
对于每一个多重线性映射 $f:V\times W \rightarrow \mathbb{R}$ ，都可以找到（在相差一个同构的意义上）唯一的一个向量空间 $V\otimes W$ ，以及映射 $g:V\times W \rightarrow V \otimes W$ 和线性映射 $h: V \otimes W \rightarrow \mathbb{R}$ ，使得 $f = h \circ g$ 。这个向量空间 $V\otimes W$ 定义为 $V$ 和 $W$ 的张量积。

这种定义方式用到了范畴论中的泛性质（universal property）。具体来说，对于同一个数学对象，我们通常有很多种不同的构造（定义）方式，但是这些不同的构造都满足一些共有性质。

因此，我们不妨把这些性质抽象出来，作为数学对象本身的定义。这些共有的性质就叫做泛性质。

用性质来定义数学对象，就免去了构造的痛苦。不同的构造在相差一个同构的意义上（up to an isomorphism）是完全等价的。这一点在后文对外积的定义中显得尤为重要。
...
>   
> 从这组基来看，你应该能对张量积有一个感性的认识：直积的维数是 3+2，而张量积的维数是 3×2。  
>   
> **评论**  
>   
...
> **评论**  
>   
> 你可能想问：“ok，我知道张量积与直积不同，毕竟它们的维数都不一样。但是直积、直和、笛卡尔积之间有什么不同？”  
>   
> 由于本篇文章的重点不是这三者，现在仅作简要的说明：  
>   
> a. 直积可以理解为群/向量空间的笛卡尔积。更技术化一点说，直积是笛卡尔积配备一个同态。  
> b. 笛卡尔积和直积的符号是一样的，都是 $\times$ ，以至于它们经常被混用。  
> c . 直和与直积的定义方式是不同的。不过即使如此，在有限次直积/直和的情况下，几乎可以把直积与直和看成是同一个东西。详情见[这篇](https://www.zhihu.com/question/666939730/answer/3627193575)。  
>   
> 张量积与这三者都不同。m 维向量空间和 n 维向量空间的张量积是 $m \times n$ 维的，而它们的直积/直和是 $m+n$ 维的。  
>   
> **评论**  
>   
> 本文对张量采取了泛性质的定义，而不是像物理书里那样，用一个莫名其妙的坐标变换来定义。这是因为多重线性映射才是张量积的本质。即，每一个多重线性映射都要“经过”张量积。  
>   
> 采取这种定义还有一个目的，是为了为后文的交替线性映射与外积作铺垫。即在后文中，我们也会使用泛性质来定义外积。
...
在物理书中， $(m,n)$ 型张量通常被定义为一个多重线性映射 $f:\underbrace{V^*\times\cdots\times V^*}_m\times \underbrace{V \times \cdots \times V}_n\rightarrow \mathbb{R}$ ，其中 $V^*$ 有 $m$ 个， $V$ 有 $n$ 个。并且把 $M$ 上所有 $(m,n)$ 型张量的集合记作 $\mathcal{T}_n^m(M)$ 。

这两种定义之间是等价的。因为对于每一个这样的多重线性映射，都能在张量积 $\underbrace{V \otimes \cdots \otimes V}_m\otimes \underbrace{V^*\otimes \cdots \otimes  V^*}_n$ 中找到唯一的一个元素与其对应（根据泛性质）。注意，在后者中， $V$ 有 $m$ 个， $V^*$ 有 $n$ 个，与前者相反。

> **例 2.2 二次型**  
>   
> 以双线性映射 $V\times V\rightarrow \mathbb{R}$ 为例，其中 $V$ 是二维线性空间。取 $V$ 的基为 $\{e_1,e_2\}$ ，向量 $v$ 在这组基下的坐标为 $(v^1,v^2)$ ，向量 $w$ 在这组基下的坐标为 $(w^1,w^2)$ 。  
>   
> 我们在线性代数中学过，这个双线性映射（二次型）可以用一个矩阵来表示： $\begin{pmatrix} v^1 & v^2 \end{pmatrix} \begin{pmatrix} a_{11} & a_{12} \\ a_{21} & a_{22} \end{pmatrix} \begin{pmatrix} w^1 \\ w^2 \end{pmatrix} \in \mathbb{R}$ ，它将两个向量映射为一个实数。  
>   
> 根据第二种定义（多重线性映射），我们可以直接定义张量 $a$ 就是这个双线性映射。  
>   
> 而根据第一种定义（泛性质），它对应 $V\otimes V$ 上唯一的一个线性泛函 $a:V\otimes V\rightarrow\mathbb{R}$ ，即 $a\in(V\otimes V)^* \cong V^* \otimes V^*$ 。于是，张量 $a$ 可以直接写成 $V^*\otimes V^*$ 这个线性空间里的一个元素： $a=a_{11}e^{1}\otimes e^{1}+a_{12}e^{1}\otimes e^{2}+a_{21}e^{2}\otimes e^{1}+a_{22}e^{2}\otimes e^{2}$ 。  
>   
> 这些定义彼此之间都是等价的，即有  
>  $\mathcal{L}(V\times V,\mathbb{R})=: L(V\otimes V,\mathbb{R})=:(V\otimes V)^*\cong V^*\otimes V^*$   
>   
> 其中 $\mathcal{L}(V\times V,\mathbb{R})$ 表示 $V\times V$ 到 $\mathbb{R}$ 的**多重**线性映射， $L(V\otimes V,R)$ 表示 $V\otimes V$ 到 $\mathbb{R}$ 的线性映射，根据泛性质，它们是一一对应的。  
>   
> 而 $L(V\otimes V,R)$ ，根据定义，就是 $V\otimes V$ 的对偶空间 $(V\otimes V)^*$ ，它与 $V^*\otimes V^*$ 自然同构： $(e_i\otimes e_j)^*\mapsto e_i^*\otimes e_j^*=e^i\otimes e^j$ ，其中 $(e_i\otimes e_j)^*$ 是 $e_i\otimes e_j$ 的对偶基。  
>   
...
正如泛性质所说，对于每一个双线性映射 $f:V\times V\rightarrow \mathbb{R}$ ，我们都能找到唯一的一个线性空间 $V\otimes V$ 以及映射 $g:V\times V\rightarrow V \otimes V$ 和映射 $h:V\otimes V\rightarrow \mathbb{R} $ ，使得 $f = h \circ g$ 。我们现在来看一下 $g$ 和 $h$ 的坐标表示。  
  
 $g$ 的坐标表示就是 $g:(v^i,v^j)\mapsto v^{ij}$ 。  
而 $h$ 的坐标表示就是 $h:v^{ij}\mapsto a_{ij}v^{ij}$ 。  
把 $h$ 和 $g$ 复合起来，也就得到了 $f:(v^i,v^j)\mapsto a_{ij}v^iv^j$ 。  
  
我们将 $g:(v^i,v^j)\mapsto v^{ij}$ 称为**并矢**。并矢也可以叫做两个向量的张量积。如果一个张量可以被写成向量的并矢，那么我们称该张量是可分的（separable）。

我们接下来看看作为线性变换的张量，它是一个 $(1,1)$ 型张量。
...
>   
> 我知道你已经晕了。没关系，我们现在用坐标的方式把上述的关系写出来： $A^i_ju^j=v^i$ ，这看起来像是一个 $(1,1)$ 型张量 $A$ 作用到一个切向量 $u$ 上，得到了另一个切向量 $v$ 。很显然，这跟线性变换是一模一样的。  
>   
>  现在回顾一下合同矩阵和相似矩阵。我们在线性代数中学过二次型（见**例 2.2**）和线性变换的矩阵，以及它们在基变换下是如何变化的，变化的规则分别对应**合同矩阵**和**相似矩阵**。实际上，合同矩阵和相似矩阵所满足的公式就是 $(0,2)$ 型张量和 $(1,1)$ 型张量所满足的坐标变换规则。

接下来，我们来看看张量的坐标变换的公式。

> **例 2.4** **坐标变换**  
>   
>  现在，我们来考察在基变换 $(e^\prime)_i = A^j_i e_j \Leftrightarrow e_i = (A^{-1})^j_i (e^\prime)_j$ 下，切向量和余切向量（乃至一般的张量）的坐标是如何变化的。  
>   
>  考虑一个切向量 $v$，它的坐标为 $v^i$，即 $v = v^ie_i$，于是有 $v = v^ie_i = v^i(A^{-1})^j_i (e^\prime)_j$。所以切向量的坐标变换公式为： $ (v^\prime)^i =(A^{-1})^j_i v^i $   
>   
>  可见，切向量的坐标变换的矩阵是基变换的矩阵的逆，因此我们也将切向量叫做逆变（Contravariant）向量  
> 。  
>   
>  下面再来考察切向量的基变换 $(e^\prime)_i = A^j_i e_j$ 对应的余切向量的基变换。由于对偶基满足关系式 $e^ie_j = \delta^i_j$ 和 $(e^\prime)^i(e^\prime)_j = \delta^i_j$，因此将 $(e^\prime)_i = A^j_i e_j$ 代入 $(e^\prime)^i(e^\prime)_j = \delta^i_j$ 得 $(e^\prime)^j = (A^{-1})^j_i e^i$。换言之，余切空间的的基变换矩阵是切空间的基变换矩阵的逆。  
>   
...
>   
>  之所以叫它单位张量，是因为如果我们把 $(1,1)$ 型张量看作线性变换，那么单位张量对应的线性变换为恒等变换，它的坐标表示对应一个单位矩阵。
...
>   
>  这是因为雅可比矩阵 $J$ 是余切空间的基变换矩阵，而后者是切空间的基变换矩阵的逆 $A^{-1}$ 。
...
>   
> 那么我们可能还想问： $V$ 与 $V^{*}$ 是自然同构的吗？毕竟对偶向量作用在一个向量上，和**内积**实在是太像了。   
>   
> 如果没有定义内积的话，答案是否定的。因为这个同构总是依赖于基的选取。   
>   
> 你可能会想，将 $V$ 的基 $\{e_i\}$ 与 $V^*$ 中的对偶基 $\{e^i\}$ 一一对应，不是能得到一个自然同构吗？非也。因为如果你改变了 $V$ 的基，那么 $V^{*}$ 中对应的对偶基也会发生变化。   
>   
> **但是**，如果定义了一个内积的话，那么在该内积的意义下， $V$ 与 $V^{*}$ 之间可以建立一个自然同构。或者说典范同构。 换言之，内积“**诱导**”了一个从 $V$ 到 $V^*$ 的典范同构。   
>   
> 为了把复数的情况也考虑进来，我们先定义厄米形式。
...
> **评论**   
>   
> 非退化的意思是，对于每一个 $x\neq 0$ ，都存在 $y \in V$ ，使得 $\langle x|y\rangle\neq 0$ 。   
>   
...
$v_i\mapsto v^j$ 就是该典范同构的坐标表示。

> 在量子力学中，这对应 $|v\rangle \mapsto \langle v|$ 。量子力学中经常考虑无穷维线性空间，无法使用上下标的记号。

由于内积是一个张量，所以我们自然地就可以定义一个由内积构成的张量场，叫做**度规张量场**，简称**度规**。
...
>   
> 实际上，张量的指标升降是度规所诱导的一个切丛与余切丛之间的音乐同构 (Musical Isomorphism）。   
>   
> 音乐同构指的是流形上每一点的切空间与余切空间之间的典范同构。典范同构是相对于流形上的某一点来说的，而音乐同构是相对于整个流形来说的。   
>   
> 在五线谱中，有着升降号 $\sharp$ 和 $\flat$ 。在微分几何中，我们借用这两个符号来表示指标的升降。也可以写成 $\sharp_g$ 和 $\flat_g$ ，用来明确表示升降是借助度规 $g$ 的。   
>   
...
>   
> **评论** “度规”的意思就是我们可以定义长度（从而，也可以定义夹角、面积、体积等等）。  
>   
...
最后，我们介绍了内积和度规，它们诱导了切空间（切丛）与余切空间（余切丛）之间的典范同构，从而允许我们对张量进行指标升降运算。

## 第三章 流形上的微分  

首先，本章介绍交替多重线性映射和外代数。

接着，基于外代数，我们就可以引入微分形式和外微分，然后回顾梯度、散度、旋度和赝矢量等概念，为引入 Hodge 对偶的概念做准备。
...
把 $n=0,\cdots,m$ 的所有 $\Lambda^n(V)$ 直和起来，我们就得到了一个外代数：
...
>   
> 量子玻色采样就涉及到对称代数。具体地，对于外代数中的行列式，可以在对称代数中找到一个对应物——积和式（Permanent）。然而，与行列式不同，计算积和式的时间复杂度是指数复杂度。而量子玻色采样可以将其降到多项式复杂度，因此被人们视作“量子霸权”（Quantum Supremacy）的一个例子。  
>   
> **评论**  
>   
> 作为物理人，你可能想问：代数是什么意思？简单来说，代数就是配备了向量乘法的向量空间（或者模，Module）。在外代数中，向量乘法是外积 $\wedge$ ，在张量代数中，向量乘法是张量积 $\otimes$ 。  
>   
> 我们在中学学过叉乘。定义了叉乘的线性空间也是一个代数。它很像是一个 3 维的外代数，但实际上，叉乘是外积加上 Hodge 对偶： $\star: V\wedge V \rightarrow V$。当我们做叉积时，我们实际上先做了一次外积，然后做了一次 Hodge 对偶。我们将在第 3.7 节介绍 Hodge 对偶。  
>   
> **评论**   
>   
> 本文对外积采用了类似于上文张量积的定义，即用范畴论中的泛性质。这样做为了强调交替多重线性映射与外积之间的关系。   
>   
> **选读 3.1**  
>   
...
>   
> 我们可以证明这个定义与上文定义的等价性。见下一节的**选读 3.2**。
...
$\bigwedge^n(T_p^*M)$ 里的元素（n-form），根据泛性质，唯一对应一个多重线性映射 $(T_pM)^{n}\rightarrow \mathbb{R}$ ，它把多个切向量变成一个实数，并且交换两个输入的切向量时，结果会多一个负号。

> **评论 第 2.5 节**介绍的某点处的微分（余切向量）其实就是一个 1-form。

如果强调研究流形上的所有点，则我们会定义流形上的 n 阶微分形式，它为 $\bigwedge^n(T^*M)$ 的**光滑**截影：$M\rightarrow \bigwedge^n(T^*M)$ 。其中 $\bigwedge^n(T^*M)$ 定义为 $\bigwedge^n(T^*M):= \{(p,\omega)\mid\,p\in M,\ \omega\in \bigwedge^n(T_p^*M)\}$ 。

用换句话， $n$ 阶微分形式将点 $p$ 映射为 $(p,\omega)$ ，其中 $\omega$ 是一个 $n$ 阶形式。

我们也可以把 $n$ 阶微分形式看作是一个 $(0,n)$ 型反对称张量场。这一点我们会在下面的**选读 3.2** 中介绍。

> **评论**  
>   
> 微分形式可以看作是“形式的场”，即流形上每一点都分配了一个该点处的形式。  
>   
...
>   
> 第二种构造是： $\omega\in\bigwedge^n(T_p^*M)$ 是一个线性映射 $\bigotimes^n(T_pM)\rightarrow \mathbb{R}$ 。在这种构造下，一个 n-form 是一个 $(0,n)$ 型张量，它把点 $p$ 处的 $(n,0)$ 型张量映射成一个实数。当然，不是随便一个 $(0,n)$ 型张量都可以（要不然和张量代数就没区别了），而是要满足反对称性，即 $\omega$ 是一个反对称 $(0,n)$ 型张量。反对称的意思是：交换两个下标/上标，结果多一个负号。  
>   
...
>   
> 不论采用以上哪种构造来定义微分形式，都是可行的，因为这些构造彼此之间是同构的。最推荐的方式是把 n-form 当成一个交替多重线性映射，这不仅切中了外积的泛性质，而且还便于计算。
...
> **评论 可定向性**   
>   
> 考虑一个莫比乌斯带（Mobius Strip），想象一个左舵的小车在上面行驶。如果让它行驶一圈回到原来的地方，你会发现它从左舵变成了右舵。这使得我们需要考虑“手性”或者“定向性”的概念。   
>   
> 我们现在定义 n 维流形的**可定向性（Orientability）**。如果一个 n 维流形上存在处处非零的微分形式，那么称该流形为可定向的。   
>   
> 回到上面的例子，我们可以证明，在莫比乌斯带上，不存在处处非零的微分形式（连续性要求我们无法从左舵突变成右舵，即无法突然变号）。因此，莫比乌斯带不可定向。  
>   
> 在可定向的 n 维流形上，我们可以选择一个**定向（Orientation）**。一个处处非零的 n 维微分形式 $o$ 给出了一个定向。  
>   
> 如果存在一个处处为正的标量场 $f$ 使得 $o_2=f o_1$ ，那么我们说 $o_1$ 和 $o_2$ 给出了同一个定向。  
>   
> 可以看出，定向只有两种，一种满足 $o_2=f o_1$ ，另一种满足 $o_3=-fo_1$ 。因此，我们实际上是把定向定义为处处非零的微分形式的等价类。  
>   
...
外微分是一个映射 $\mathrm{d}: \bigwedge^n(T_p^*M)\rightarrow \bigwedge^{n+1}(T_p^*M)$ ，它作用到一个 n 阶微分形式： $\varphi=\sum_{I}f_I\mathrm{d}x^{I}$ 上，得到一个 (n+1) 阶微分形式：
...
>   
> 令 $F=\sharp \star f$ ，于是我们可以把 $\star\mathrm{d} f =\nabla \cdot (\sharp\star f)$ 写成：  
>   
...
现在，我们回顾一下矢量微积分的两个定理： $\nabla\cdot(\nabla\times A)=0$ 和 $\nabla\times (\nabla A)=0$ 。实际上它们只不过是三维流形上的 $\mathrm{d}^2=0$ 的表现形式而已。推导留给读者。
...
实际上，某一点处的磁场并非一个 vector，而是一个 bivector。即 $B\in (T_pM)\wedge (T_pM)$ 。它的基为 $\partial_x\wedge \partial_y,\,\partial_y\wedge \partial_z,\,\partial_z\wedge \partial_x$ 。
...
顺便一提，电场也是一个 bivector，它的基为 $\partial_t\wedge \partial_x,\,\partial_t\wedge \partial_y,\,\partial_t\wedge \partial_z$ ，有一个时间维度。之所以它看起来不是“赝矢量”，是因为我们只考虑了空间反演变换，没有考虑时间反演变换。

赝向量不是天然存在于流形上的。只有配备了度规，我们才能将微分形式通过音乐同构升为赝向量场。实际上，电磁场本质上是微分形式（余切张量场），而不是切向量场。电磁场定义为 4-势 $A$ 的外微分 $F=\mathrm{d}A$ ，见下文的**例 3.7**。
...
>   
> 也就是说，我们需要请回我们的老朋友——度规。在**第 2.9 节**中，我们通过内积诱导了从切丛 $TM$ 到余切丛 $T^*M$ 的典范同构。现在，我们通过度规来诱导从 $\bigwedge^n(T^*M)$ 到 $\bigwedge^{m-n}(T^*M)$ 的典范同构，它就是 Hodge 对偶。  
>    
> **评论**  
>   
> 以下所有推导，不仅适用于 k 阶形式 $\alpha \in \bigwedge^k(T_p^*M)$ ，而且也适用于 k-vectors： $v\in\bigwedge^k(T_pM)$ 。

给定一个黎曼流形，我们已知 $V$ 上的一个内积 $V\times V\rightarrow \mathbb{R}$ ，

现在，我们需要通过它来诱导出一个 $\bigwedge^k(V)$ 上的内积 $\bigwedge^k(V)\times \bigwedge^k(V)\rightarrow \mathbb{R}$ 。

记 $\alpha = \alpha_1\wedge\alpha_2\wedge \cdots\wedge \alpha_k \in \bigwedge^k(V)$ ， $\beta = \beta_1\wedge\beta_2\wedge \cdots\wedge \beta_k \in \bigwedge^k(V)$ ，我们定义 $\bigwedge^k(V)$ 上的内积 $\langle\alpha\mid\beta\rangle$ 如下：
...
之所以要这么定义，是因为我们希望这个内积仍然保持外代数的结构。

定义了 $\bigwedge^k(V)$ 上的内积之后，我们就可以导出从 $\bigwedge^k(V)$ 到 $\bigwedge^{n-k}(V)$ 的典范同构。接下来，我们给出这个同构，也就是霍奇对偶的定义：
...
其中 $\langle\cdot\mid\cdot\rangle$ 是 $\bigwedge^n(T_p^*M)$ 上的内积； $\omega_g$ 是单位正交基 $\mathrm{d}x^1\wedge \mathrm{d}x^2\wedge\cdots\wedge\mathrm{d}x^m$ 构成的 $m$ 阶微分形式，也称作单位体积形式，顾名思义，它的体积为 1： $\langle \omega_g | \omega_g\rangle = 1$ 。

我们再来用自然语言转述一遍：

**在** $n$ **维流形上，** $k$ **阶形式** $\beta$ **的霍奇对偶** $\star \beta$ **是这样一个** $(n-k)$ **阶形式，使得对于任意一个** $k$ **阶形式** $\alpha$ **，** $\alpha$ **与** $\star \beta$ **的外积（的体积）都等于** $\alpha$ **与** $\beta$ **的内积。**

> **例 3.6**  
>   
> 在三维流形上，给定 $T_p^*M$ 上的内积 $\langle \mathrm{d}x^i\mid \mathrm{d}x^j\rangle=\begin{cases} 1, &i= j\\ 0, & i\neq j \end{cases}$ 及其诱导的 $\bigwedge^k(T_p^*M)$ 上的内积，那么  
...
> **评论**  
>   
> 在讨论 Hodge 对偶的时候，我们总是已经给定了一个内积。或者说，内积是我们的先验知识。从这个的意义上来说，Hodge 对偶是一个自然或者典范的同构。  
>   
...
> 它实际上是 4-势 的外微分：  
>  $F = \mathrm{d}A$   
> 其中 $A = -\phi/c \,\mathrm{d}t + A_x \mathrm{d}x + A_y \mathrm{d}y + A_z \mathrm{d}z$   
> 请读者自行验证。  
>   
...
首先，我们引入交替多重线性映射，并基于它定义了外积。把 0 阶、1 阶、...、n 阶的外积直和起来，我们就得到了外代数。
...
在第三章中，我们研究了外微分（或称**外导数**，因为其英文名是 exterior **derivative**）。在本章中，我们介绍另外两种**导数**，即**协变导数**和**李导数**。  
  
协变导数与李导数都是一个张量场关于另一个切向量场的导数，即，它们都表示张量沿着另一个切向量的方向的变化。它们的区别在于，李导数依赖于另一个切向量场的邻域性质，而协变导数不依赖。但是，协变导数需要配备一个度规才能被唯一确定下来。
...
其实，协变导数是作用在两个向量场上的： $\nabla_W A$ ，它给出另一个向量场。对应到上文， $u$ 属于向量场 $A$ ， $v$ 属于向量场 $W$ 。

协变导数 $\nabla_W$ 不仅可以作用在向量场 $A$ 上，还可以推广到作用在一般的张量场上（包括标量场，因为标量场也是一种特殊的张量场）。现在我们给出协变导数的定义：
...
>   
>  但是有些作者认为，在给定了坐标系的意义下，可以说克氏符号是一个张量，或者说是“坐标系依赖的张量”。这样的说法不太严谨，因此本文不采用。
...
>   
>  从上式可以看出，协变导数 $\nabla_W A$ 和对分量的导数 $W^k\partial_k A$ 相差了一个克氏符号项。
...
>   
>  注意，在上式中，我们对 $(1,1)$ 型张量进行了缩并运算。因此等式左边的 $\mathrm{d}x^i(\partial_j)$ 是一个常标量场（要么恒为 1，要么恒为 0），从而等式左边的 $ \nabla_k(\mathrm{d}x^i(\partial_j))$ 是一个零标量场。  
>   
>  移项（并考虑缩并运算）得  
>   
...
>   
> 值得注意的是，一般而言 $VW : VW(f)=V(W(f))$ 不是一个向量场。但是 $VW-WV$ 的确是一个向量场，见**第 4.8 节**。  
>   
>  **评论**   
>   
...
>  **评论**  
>   
>  无挠性还可以用如下方式定义：$\nabla_a\nabla_b f=\nabla_b\nabla_a f$，其中 $f\in \mathcal{F}(M)$ 是一个标量场。这个定义与上文的定义等价。

加上无挠性之后，我们就限制了可能的协变导数的数量，但是我们还不能将其限制到一个。接下来我们考虑度规，从而将可能的协变导数限制到一个。
...
> **例 4.4**  
>   
>  推导无挠且保度规的协变导数的克氏符号。换言之，写出 $\Gamma^i_{ij}$ 关于 $g_{ij}$ 的表达式。  
>   
>  **解**：   
>   
>  根据 **例 4.3**，我们可以写出度规张量的协变导数：  
>   
>  $ \nabla_k g_{ij} = \partial_k g_{ij} - \Gamma^l_{ik}g_{lj} - \Gamma^l_{jk}g_{il} $   
>   
>  我们现在定义 **第一类克氏符号（Christoffel Symbol of the first kind）** 为 $\Gamma_{ijk} = g_{il}\Gamma^l_{jk}$。之前介绍的克氏符号 $\Gamma^{i}_{jk}$ 属于 **第二类克氏符号（Christoffel Symbol of the second kind）**。  
>   
>  定义了第一类克氏符号 $\Gamma_{ijk}$ 之后，上式就可以写为：  
>   
...
可见，在给定无挠性和保度规性之后，协变导数可以被唯一确定下来。与该协变导数对应的联络 $\Gamma^i_{ij}$ 叫做 **黎曼联络（Riemann Connection）** 或者 **列维-奇维塔联络（Levi-Civita Connection）** 。
...
里奇张量是黎曼曲率张量经过缩并得到的。至于 $\alpha$ 为什么和 $\mu$ 缩并，而不是和 $\nu$ 缩并，是因为这两者只相差了一个负号，和 $\mu$ 或 $\nu$ 缩并都可以。

而如果让 $\alpha$ 和 $\beta$ 缩并，则会得到零，证略。
...
> 广义相对论的核心——爱因斯坦场方程（Einstein Field Equations），给出了时空的曲率和物质运动之间的关系。前者描述时空如何弯曲，后者描述时空中能量和动量的演化。
...
其中 $\phi^*$ 叫做拉回（pull-back），顾名思义，它将 $f$ 的定义域从 $N$ 上“拉回”到 $M$ 上。

> **评论**  
>   
>  有一点需要注意，我们现在并没有假定 $\phi$ 是一个双射。实际上，$\phi$ 既不一定是单射，也不一定是满射。到目前为止，我们对 $\phi$ 的唯一要求就是它是光滑的。  
>   
...
> **评论**  
>   
>  注意到在这个定义中，我们不仅可以定义**某一点处**的余切向量的拉回，还可以对流形 $N$ 上的**所有**余切向量定义。换言之，即使在 $\phi$ 不是微分同胚的情况下，我们也可以定义余切向量场的拉回。  
>   
...
无数条积分曲线很像一条河流。如果我们“随波逐流”，我们就会从流形上的一点“漂流”到另一点：

$$ \Phi_t: M\rightarrow M \quad \gamma(0)\mapsto \gamma(t) $$

$\Phi_t$ 是一个微分同胚，证略。我们可以把 $\Phi_t$ 叫做一个“流”。

既然每一点都会随着积分曲线漂流到另一点，那么每一点处的张量也会随着点的流动而流动。这个想法应该怎么表达呢？上一节介绍的**拉回映射**此时就派上用场了：
...
> **评论**  
>   
> 最后，我们用李导数来重新定义一下散度。在**第 3.5 节**我们定义散度为  
>   
...
### 4.9 [V,W] 的几何意义、和乐（Holonomy）  

在 **第 4.5 节** 中，我们通过考察一个张量先沿着坐标线 $\mu$ **平移**再沿着坐标线 $\nu$ 平移很短一段距离，和先沿着 $\nu$ 再沿着 $\mu$ 平移很短一段距离，有什么区别。通过这种方法，我们导出了黎曼张量。这是针对**协变导数**而言的。

我们现在想要知道，对于**李导数**而言，一个点先沿着向量场 $V$ **流动**，再沿着向量场 $W$ 流动很短一段距离，和先沿着 $V$ 再沿着 $W$ 流动很短一段距离有什么区别。这就引出了和乐（Holonomy）的概念。
...
如果坐标系的对易子 $[e^i, e^j] = 0$，则称这是一个和乐的（Holonomic）坐标系，否则是 Non-holonomic 的坐标系。一个 Non-holonomic 的坐标系不能被表示为沿坐标线的求导。这是因为沿坐标线的求导满足 $[\partial^i, \partial^j]=0$，一定是和乐的。
...
那么，对于李导数，我们可不可以也做出这样的约束呢？答案是，这样做会产生“过度约束”，即给定一个度规，我们不能保证存在一个李导数，使得该李导数保度规。满足这种性质的李导数所对应的向量场叫做 Killing 向量场。记一个 Killing 向量场为 $\xi$，它满足 Killing 方程：
...
由于 $g_{ij}=g_{ji}$，所以 Killing 方程组共有 $n(n+1)/2$ 个方程。然而，自由变量只有 $n$ 个（即 $\xi^k, k=1,...,n$），所以这是一个“过度约束”的方程组，并不能保证有解。
...
李导数描述了流形上向量场的“流动”，它引出了 Holonomy 和 Killing 向量场，前者刻画流动是否与路径相关，后者刻画流动是否具有度规不变性。用李导数我们可以更加优雅地定义曲率、挠率、散度等概念。另外，它还与李群和李代数有关，不过本文暂不介绍。
...
其次，我们给出微分形式的积分的一个定义，然后介绍大名鼎鼎的斯托克斯定理。这个定理给出了边界算子和外微分算子之间的关系。

最后，既然已经介绍了边界算子、外微分算子、以及斯托克斯定理，那么我们就不得不提到同调和上同调。同调和上同调理论将以上所有概念都以一种不可思议的方式联系在了一起，并用它们来处理流形上的“孔洞”。
...
也可以这么理解：一个 m-cell 为流形 $M$ 上的一个 m 维区域中的每个点分配一个参数： $\bm{u}\in[0,1]^m$ 。
...
还有一种参数化的方式，叫做 m 维单形（m-simplex, pl. m-simplices）。与 m 维胞腔使用的矩形区域 $[0,1]^m$ 不同，它使用的是三角形区域 $\{(\theta_0,\theta_1,\cdots,\theta_m)\mid\sum_i\theta_i=1,\, \theta_i\ge 0\}$ 。这个三角形区域叫做 $\mathbb{R}^m$ 中的标准单形。它关于 $\varphi$ 在流形上的像，就是流形上的一个 m-simplex。
...
> **评论**  
>   
>  现在我们来考虑如何定义一块区域的边界。有了胞腔/单形和链的概念之后，我们就可以来形式化地定义“边界”。
...
>   
>  提示：由于 $(\operatorname{div} V)\omega_g =: \mathcal{L}_V \omega_g$，根据斯托克斯定理，只要证明 $\mathcal{L}_V \omega_g = \mathrm{d}(V^i\mathrm{d}\Sigma_i)$ 即可。
...
注意到，在没有洞的流形上，一个没有边界（用自然语言来说就是闭合/首尾相接）的超曲面/曲线一定是另一个区域的边界，换句话说：一个边界为零（见**第 5.2 节**）的 p-链 总是另外一个 (p+1)-链的边界。
...
甜甜圈的同调群是 ZxZ，从图中可以直观地看出，有两种不同的环绕方法

  
  

我们再来看看上同调所关心的事情。在物理中，我们通常默认一个闭形式也是一个恰当形式。例如，我们由电场的旋度（见**第 3.4-5 节**）为零推导出电势的存在，以及由磁场的散度（见**第 3.4-5 节**）为零推导出磁势的存在。
...
> **评论**  
>   
>  想象一个圆柱体的侧面（$\mathbb{R}^1 \times S^1$），上面有一个均匀环绕它一圈的电场。可以证明，这样的电场是无散的，但它却不是一个保守场：一个电子朝电场的方向绕一圈回到原来的位置，能量会减少。此时电势是不良定义的。  
>   
...
> **评论**  
>   
>  当然，反过来，恰当形式一定是闭形式，这是因为 $\mathrm{dd} = 0$（**定理 3.1**）。

可以定义（de Rham）上同调群为“所有恰当形式”商掉“所有闭形式”，即：
...
$H^p(M)$ 叫做 $p$ 阶上同调群，其中的元素叫做上同调类。上同调群告诉我们，不是恰当形式的闭形式有多少类。
...
> 严格来说，这里的上同调是奇异上同调 $H^k(M;\mathbb{Z})$ 而不是 de Rham 上同调 $H^k_{\text{dR}}(M)$ ，后者同构于 $H^k(M;\mathbb{R})$ 。
