...
对于单个量子比特（Qubit）而言， $d=2$ ，但对于 $n$ 个量子比特所组成的系统，则是 $d=2^n$ 。对于 $n$ 个 k 维的 qudit，则是 $d=k^n$ 。可见，希尔伯特空间维数 $d$ 随粒子数 $n$ 的增加而指数级增长。
...
本文主要使用迹距离 $||\rho - \sigma||_1$ 作为误差（额外的常数 1/2 在 scaling 下不重要）。

### 2.2 样本复杂度的上界  

直觉上来看，为了重建量子态 $\rho$ ，样本复杂度随 $d$ 的变化至少是 $\sim d^2$ ，或写成 $\Omega(d^2)$ ，这是因为我们至少需要 $\sim d^2$ 个测量基（POVM）元素，才能使测量基是信息完全（informationally complete）的。另一方面，我们要多次测量从而使估计量逼近真实值。为了使两者的误差小于 $\epsilon$ ，样本复杂度与误差 $\epsilon$ 之间很可能是反比的关系 $\sim O(\epsilon^{-m})$ 。

因此，我们猜想，样本复杂度为 $\boxed{ N = O(d^l/\epsilon^m) }$ ，其中 $l \ge 2$ 。接下来，我们来研究不同测量方式下的样本复杂度。

### 2.3 泡利测量的复杂度  

接下来我们给出样本复杂度的一个较为宽松的上界[[1]](#ref\_1)，它是由前文提到，也是人们最常用的泡利测量（Pauli measurements）给出的。
...
人们已经证明，对于这种最一般的测量方式，样本复杂度的确界为 $\boxed{\Theta(d^2 / \epsilon^{2})}$ [[4]](#ref\_4) [[5]](#ref\_5)。这一点直到 2016 年才被证明。
...
这揭示了一个似乎惊人的事实：如果我们只关注算符的期望值，而不是量子态本身，那么样本复杂度可以大大降低到对数级别，完全失去了 $d^{\sim2-3.5}$ 的依赖。
...
其中 $||\cdot||_{\text{shadow}}$ 的具体形式取决于测量手段。这个结果来自于 2020 年 Huang et al. 的著名的“经典阴影”（Classical Shadows）[[6]](#ref\_6)。

从第二节的分析可以看出，这一复杂度并不令人意外。那么，为什么“经典阴影”在近年来会变得如此热门呢？

个人认为，这是因为：以往关于样本复杂度的研究，动机多在于缩小理论上的上界，而不是物理应用；即便存在测量方法，在实验中也难以实现，更多属于数学家口中的“理论上存在”。而经典阴影提供了一种简单、具体且有效的测量方法：实现步骤十分明确，实验物理学家可以开箱即用。另外，对数复杂度本身就是一个很大的噱头（即便它并不像乍一看那样令人惊讶）。

实际上，早在 2017 年，Scott Aaronson 就提出了阴影层析（Shadow Tomography）[[7]](#ref\_7)，也指出了对数复杂度 $\log(M)$ 。只不过他考虑的是 $M$ 个 two-outcome POVM 的概率 $\operatorname{tr}[\rho E_i]$ ，而不是 $M$ 个一般的物理量算符 $\langle O_i \rangle = \operatorname{tr}[\rho O_i]$ 。之所以 Shadow Tomography 的工作没有 Classical Shadows 那么受关注，也是因为它给出的测量手段比较复杂。

> 趣事：Scott Aaronson 的博客很有意思，例如这篇争议很大的：[Shtetl-Optimized » Blog Archive » Cargo Cult Quantum Factoring](https://scottaaronson.blog/?p=6957)
...
**1.** 测量量子态得到所谓的“Classical Shadows”，并存储到经典计算机中

**2.** 从存储的 Classical Shadows 出发，通过一定的算法同时估计 $M$ 个算符的期望值。
...
其中 $d=2^n$ 为希尔伯特空间的维数。这个 $\mathcal{M}(\rho)$ 其实就是一个 depolarizing channel。

于是可以直接得到 $\mathcal{M}$ 的逆为：

$$\boxed{ \mathcal{M}^{-1}(X)=(d+1) X - \operatorname{tr}[X]\mathbb{I} }$$

于是我们可以把 $\mathcal{M}^{-1}(U|b\rangle\langle b| U^\dagger)$ 作为 $\rho$ 的估计量：
...
为了避免离群值的影响，给成功率兜底，实际需要使用平均数中位数（Median of means）法，将 $S(\rho,N)$ 分成 $K$ 份，对每一份求 $\hat{o_i}$ ，然后取 $K$ 个结果的中位数。

### 3.2 Haar 测度与 Unitary t-design  
...
> **定理：t 阶矩** $M^{(t)}$ **是投影到 commutant 上的投影算子**  
>   
...
在第二节中，我们使用了 Hoeffding 不等式给出样本复杂度的宽松上界。这里，我们使用平均数中位数估计（Median of means estimation）来给出样本复杂度的宽松上界。
...
实践中更常用的是局部测量，即对各个 qubit 分别进行单比特门操作，然后在计算基下测量。通常这些门被选取为单比特的 Clifford 群，它们能够实现泡利基（也就是 $X, Y, Z$ ）下的测量。在这种情况下，样本复杂度还取决于待测算符的 locality。这是很显然的：如果待测算符是高度纠缠的，那么你很难指望很少的局域测量能给出纠缠的信息。

(未完待续)

### 3.8 去随机化（Derandomization）  

经典阴影与传统的 FST 非常不同。在传统的 FST 中，我们在每个测量基下都测量一样多的样本，这样能够保证误差在一定程度上更加均匀。而在经典阴影中，由于测量基是随机选取的，所以不同测量基下的样本数也不同。
