...
已知一系列测量算符 \(\{\Pi_1,\cdots,\Pi_n\}\) 及其对应的测量概率 \(p_k=\operatorname{Tr}[\rho \Pi_k]\) ，求量子态 \(\rho\) 。

换句话说，在 \(p_k=\operatorname{Tr}[\rho \Pi_k]\) 这 n 个公式里面，已知 \(p_k\) 和 \(\Pi_k\) ，求 \(\rho\) 。

> 反过来，如果已知 \(\rho\) 和 \(p_k\) 求 \(\Pi_k\) ，那么就叫 Detector tomography。不过这不是本篇文章的内容。

为什么要关心 state tomography？因为我们想要知道实验上制备的量子态具体长啥样。方法就是 state tomography。

从公式 \(p_k=\operatorname{Tr}[\rho \Pi_k]\) 中可以看出，如果 \(\rho\) 有 \(n\) 个自由度，那么我们也至少需要 \(n\) 个 测量算符，而且这些测量算符之间还要“相互独立”，这个条件叫做 tomographical completeness 或者 informational completeness。
...
但问题是，在实验上，我们不是直接测量概率，而是测量结果的计数。我们如果想要知道概率 \(p_Z=\operatorname{Tr}[\rho (\mathbb{I}+Z)/2] = \langle0|\rho|0\rangle\) ，就必须要利用计数 \(p_Z=\frac{N_Z}{N_Z+N_{-Z}}\) ，而测量 \(N_{-Z}\) 又需要新的一个 测量算符 \((\mathbb{I}-Z)/2\) 。
...
好了，废话少说，我们来仔细研究一下一个 qubit 的 tomography，这对后面推广到多个 qubit 有帮助。
...
> 这里采用了连续的正态分布而不是离散的泊松分布，是因为在数量较大时（实验上通常为至少几百个到几千个），正态分布可以很好地近似泊松分布。  
>   
...
此时我们只要求 \((x_0,y_0,z_0) = \underset{|x|,|y|,|z|\le1}{\operatorname{arg\,min}} \mathcal{L}(x,y,z)\) 即可。
...
其中 \(\sigma_i,\,i=0,1,2,3\) 分别为恒等矩阵和泡利矩阵 \(\mathbb{I},X,Y,Z\) 。且 \(c_{i_1i_2\cdots i_n}\) 叫做 correlators。
...
相较于将 correlators \(c_{i_1i_2\cdots i_n}\) 作为参数且约束它们的绝对值不大于一而言，T(t) 这种参数化方式的好处是把即将要进行的极大似然估计从有约束的问题变成了无约束的问题（我们对 t 没有任何约束）。
...
一些有趣的光子态的 Wigner 函数。左图为压缩态，右图为薛定谔猫态。
...
其中 \(\lambda_k = N \operatorname{Tr}[\rho(\mathbf{t}) \Pi_k]= N \hat{p}_k(\mathbf{t})\) 。

对似然函数求导找极小值，可以发现极小值在 \(\hat{p}_k(\mathbf{t}) = p_k\) 时取到。这说明采用泊松分布也是正确的。但是泊松分布的似然函数里有对数，而正态分布的似然函数里都是多项式，后者对于计算机更友好，收敛更快一些。所以我们一般采用正态分布对应的似然函数。
