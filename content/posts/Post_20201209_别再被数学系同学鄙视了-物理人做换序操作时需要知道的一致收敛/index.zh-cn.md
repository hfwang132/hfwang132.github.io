...
> **例 1.1**  
> 为了使读者看出，一致收敛比普通的逐点收敛要强，举一个简单的例子。设部分和函数（或函数列的一般项）是 \(S_n(x)=x^n\) ，显然，在区间 \([0,1]\) 上，它逐点收敛于这个函数 ：\(\begin{equation}  S(x)=  \begin{cases}  0&0\le x<1\\  1&x=1  \end{cases}  \end{equation}\)   
